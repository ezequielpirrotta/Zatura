...
class Engine
{
    ...
    public static void DrawText(string text,
        int x, int y, int width, int height, byte r, byte g, byte b, Font f)
    {
        DrawText(text, x, y, width, height, r, g, b, f.ReadPointer());
    }
    public static void DrawText(string text,
    ...
    public static void DrawText(string texto,
        int x, int y, int width, int height, byte r, byte g, byte b, IntPtr fuente)
    {
        Sdl.SDL_Color color = new Sdl.SDL_Color(r, g, b);
        IntPtr textAsImage = SdlTtf.TTF_RenderText_Solid(
            fuente, texto, color);
    ...
    /*
    // Actualizar posición del fondo para el efecto de movimiento
    backgroundX -= ScrollSpeed;

    // Resetear la posición del fondo cuando sale de la pantalla
    if (backgroundX <= -ScreenWidth)
    {
        backgroundX = 0;
    }

    // Renderizar el fondo dos veces para el efecto de "wrapping"
    SDL.SDL_Rect srcRect = new SDL.SDL_Rect { x = 0, y = 0, w = ScreenWidth, h = ScreenHeight };
    SDL.SDL_Rect destRect1 = new SDL.SDL_Rect { x = backgroundX, y = 0, w = ScreenWidth, h = ScreenHeight };
    SDL.SDL_Rect destRect2 = new SDL.SDL_Rect { x = backgroundX + ScreenWidth, y = 0, w = ScreenWidth, h = ScreenHeight };

    SDL.SDL_BlitSurface(background, ref srcRect, screen, ref destRect1);
    SDL.SDL_BlitSurface(background, ref srcRect, screen, ref destRect2);

    // Actualizar la pantalla
    SDL.SDL_Flip(screen);

    // Control de la velocidad de fotogramas
    SDL.SDL_Delay(16); // Aproximadamente 60 FPS*/
